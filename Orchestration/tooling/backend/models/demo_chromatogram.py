...
def run_chromatogram_demo(checkpoint_path: str, output_dir: str, N: int = 4096):
    ...
    Compares classical BEADS vs LBEADS-NET on legacy chromatogram data and
    saves output plots to output_dir.

    Args:
        checkpoint_path: Path to a .pth checkpoint file.
        output_dir: Directory where output plots are written.
        N: Model signal length (must match training).
    ...
    # Locate legacy chromatogram data
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_dir = os.path.normpath(os.path.join(script_dir, '..', '..', '..', '..',
                                              'Implementations', '0. BEADS', 'data'))
    if not os.path.isdir(data_dir):
        # Fallback: try relative to source implementations
        data_dir = os.path.normpath(os.path.join(script_dir, '..', '..', '..', '..',
                                                  '..', 'Implementations', '0. BEADS', 'data'))
    if not os.path.isdir(data_dir):
        print(f"Legacy chromatogram data directory not found. Tried: {data_dir}")
        return []
    ...
    output_files = ['demo_chromatogram.png']
    return output_files
