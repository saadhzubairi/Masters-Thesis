...
class LBEADS_NET_Fast(nn.Module):
    ...
    def __init__(self, N, d=1, fc=0.006, num_layers=10,
        ...
        # Store dense versions for differentiable operations
        self.register_buffer('A_dense', torch.tensor(A.toarray(), dtype=torch.float64))
        self.register_buffer('B_dense', torch.tensor(B.toarray(), dtype=torch.float64))
        BTB = B.T @ B
    ...
    def forward(self, y, return_intermediate=False):
        ...
        # Initialize x = 0 (sparse initialization)
        ...
        for k in range(self.num_layers):
            ...
            # Gradient of data fidelity: ||y - x - f||^2
            # Approximate f as low-pass filtered (y - x)
            # For speed, use simple smoothing instead of full filter solve
            residual = y - x
            
            # Compute gradient of smoothness penalties on x
            # D1 penalty gradient
            Dx1 = self.diff1(x)
            ...
            # Gradient step on smoothness
            x_grad = x - step_size * (grad_D1 + grad_D2)
            
            # Proximal step: asymmetric soft thresholding for sparsity
            # This encourages x to be sparse and non-negative (peaks are positive)
            x = self.asymmetric_soft_threshold(
                x_grad + step_size * residual,  # Include data term
                lam0 * step_size,
                r
            )
        ...
        # Compute baseline: f = y - x - H(y - x) where H is high-pass
        residual = y - x
        
        # Proper baseline using sparse solver (only at inference, not per-layer)
        f_list = []
        for b in range(batch_size):
