...
class ExperimentConfig:
    ...
    BEADS_FC: float = 0.01           # Filter cutoff frequency (higher to preserve peaks)
    BEADS_R: float = 6.0             # Asymmetry ratio
    BEADS_LAM0: float = 0.3          # Asymmetric penalty weight (lower = more signal)
    BEADS_LAM1: float = 3.0          # First derivative penalty (lower = less smoothing)
    BEADS_LAM2: float = 3.0          # Second derivative penalty
    BEADS_NIT: int = 50              # Number of iterations (more for convergence)
    
    # LBEADS-NET parameters
    LBEADS_NUM_LAYERS: int = 30      # Number of unrolled layers
    LBEADS_SHARED_PARAMS: bool = True  # Share parameters across layers
    LBEADS_MODEL_PATH: str = None    # Path to trained model (None = use init)
...
def create_lbeads_model(N: int, config: ExperimentConfig = ExperimentConfig):
    """
    Create LBEADS-NET model (Fast or regular depending on checkpoint).
    ...
    Returns:
        LBEADS_NET or LBEADS_NET_Fast model
    """
    # Check if we have a trained model with saved config
    if config.LBEADS_MODEL_PATH and os.path.exists(config.LBEADS_MODEL_PATH):
        ...
        model_type = saved_config.get('model_type', 'LBEADS_NET')
        
        if model_type == 'LBEADS_NET_Fast':
            # Use Fast model for trained weights
            ...
            # Use regular LBEADS_NET
            model = LBEADS_NET(
                N=N,
                d=saved_config.get('d', config.BEADS_D),
                fc=saved_config.get('fc', config.BEADS_FC),
                num_layers=saved_config.get('num_layers', config.LBEADS_NUM_LAYERS),
                shared_params=saved_config.get('shared_params', config.LBEADS_SHARED_PARAMS),
                init_lam0=config.BEADS_LAM0,
                init_lam1=config.BEADS_LAM1,
                init_lam2=config.BEADS_LAM2,
                init_r=config.BEADS_R
        ...
        print(f"Loaded trained model from {config.LBEADS_MODEL_PATH}")
        ...
        model = LBEADS_NET(
            ...
            shared_params=config.LBEADS_SHARED_PARAMS,
            init_lam0=config.BEADS_LAM0,
            init_lam1=config.BEADS_LAM1,
            init_lam2=config.BEADS_LAM2,
            init_r=config.BEADS_R
        )
        print("Using untrained LBEADS-NET (will replicate BEADS)")
...
def main():
    ...
    parser.add_argument('--model-path', type=str, default=None,
                        help='Path to trained LBEADS-NET model')
