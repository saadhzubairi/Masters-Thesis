...
class TrainingConfig:
    ...
    # Initialization (tuned for synthetic data)
    INIT_D: int = 1
    INIT_FC: float = 0.01            # Higher cutoff to preserve peaks
    INIT_R: float = 6.0
    INIT_LAM0: float = 0.3           # Lower for more signal recovery
    INIT_LAM1: float = 3.0           # Lower for less aggressive smoothing
    INIT_LAM2: float = 3.0
...
def create_model(config: TrainingConfig) -> LBEADS_NET_Fast:
    """Create LBEADS-NET Fast model for training (vectorized, much faster)."""
    ...
    model = LBEADS_NET_Fast(
        ...
        init_step_size=0.1  # Learnable step size
    )
    
    return model


def train_epoch(model: LBEADS_NET_Fast, dataloader: DataLoader, 
...
def validate(model: LBEADS_NET_Fast, dataloader: DataLoader,
...
def print_params(model: LBEADS_NET_Fast, title: str = "Model Parameters"):
