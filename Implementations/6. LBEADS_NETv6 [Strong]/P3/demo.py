...
def _generate_dense_narrow_peak_signal(
    generator: SyntheticDataGenerator,
    noise_level: float = 0.01,
) -> SyntheticSignal:
    """
    Generate synthetic signal with dense, very narrow, overlapping peaks.
    """
    peak_count = int(generator.rng.integers(30, 41))
    overlap_count = int(max(10, peak_count // 3))
    base_count = int(max(1, peak_count - overlap_count))

    center_margin = 120
    low = center_margin
    high = generator.N - center_margin + 1
    if high <= low:
        low, high = 0, generator.N

    base_centers = generator.rng.integers(low, high, size=base_count)
    overlap_centers = []
    for _ in range(overlap_count):
        anchor = int(base_centers[int(generator.rng.integers(0, base_count))])
        jitter = int(generator.rng.integers(-5, 6))
        c = int(np.clip(anchor + jitter, low, high - 1))
        overlap_centers.append(c)

    centers = np.concatenate([base_centers, np.asarray(overlap_centers, dtype=np.int64)])
    overlap_flags = np.concatenate([
        np.zeros(base_count, dtype=bool),
        np.ones(overlap_count, dtype=bool),
    ])
    order = np.argsort(centers)
    centers = centers[order]
    overlap_flags = overlap_flags[order]

    x_true = np.zeros(generator.N, dtype=np.float64)
    peak_info = []
    for center, is_overlap in zip(centers, overlap_flags):
        if is_overlap:
            amplitude = float(generator.rng.uniform(0.05, 0.40))
        else:
            p = float(generator.rng.random())
            if p < 0.25:
                amplitude = float(generator.rng.uniform(0.80, 1.00))
            elif p < 0.75:
                amplitude = float(generator.rng.uniform(0.25, 0.80))
            else:
                amplitude = float(generator.rng.uniform(0.06, 0.25))

        rise_w = int(generator.rng.integers(1, 5))
        decay_w = int(generator.rng.integers(2, 9))
        plateau_w = int(generator.rng.integers(0, 2))

        peak = generator.beads_peak(generator.N, int(center), amplitude, rise_w, decay_w, plateau_w)
        x_true += peak
        peak_info.append({
            "type": "linear",
            "center": int(center),
            ...
            "is_overlap_peak": bool(is_overlap),
        })
    ...
    f_true, baseline_meta = generator.generate_baseline()
    noise, noise_meta = generator.generate_noise(noise_level)

    # Keep normalization/data structure aligned with training synthetic signals.
    ...
    metadata = {
        ...
        "peaks": {
            "num_peaks": int(peak_count),
            "shape_mode": "linear_dense_narrow",
            "overlap_count": int(overlap_count),
            "config": {
                "num_peaks_range": (30, 40),
                "rise_width_range": (1, 4),
                "decay_width_range": (2, 8),
                "plateau_width_range": (0, 1),
                "overlap_jitter": (-5, 5),
            },
            "peaks": peak_info,
        ...
        "derivative_stats": {
            ...
            "diff1_threshold": float(d1_thr),
            "diff2_threshold": float(d2_thr),
        },
    }
...
def main():
    ...
    synthetic_peak_config = {
        "num_peaks_range": (30, 40),
        "rise_width_range": (1, 4),
        "decay_width_range": (2, 8),
        "plateau_width_range": (0, 1),
        "overlap_mode": "forced clusters with short-on-tall overlays",
    }
    ...
    for _ in range(num_test_samples):
        ...
        signal = _generate_dense_narrow_peak_signal(generator=generator, noise_level=noise_level)
    ...
    print(f"  Synthetic peak config: {synthetic_peak_config}")
