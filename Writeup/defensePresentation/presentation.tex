...
\begin{tikzpicture}[scale=0.9, transform shape]
  % --- Baseline (dashed) ---
  \draw[dashed, thick, steelblue, domain=0:12, samples=100]
    plot (\x, {0.3*sin(15*\x)+0.15*cos(25*\x)+0.8}) node[right]{\small Baseline drift $\mathbf{f}$};

  % --- Peaks ---
  \draw[forestgreen, very thick, domain=0:12, samples=200]
    plot (\x, {
      1.8*exp(-(\x-2.5)^2/0.3)
      + 1.2*exp(-(\x-4.5)^2/0.15)
      + 2.2*exp(-(\x-7)^2/0.5)
      + 0.9*exp(-(\x-9.5)^2/0.2)
      + 1.4*exp(-(\x-10.8)^2/0.25)
    });
  \node[forestgreen, right] at (12, 0.5) {\small Peaks $\mathbf{x}$};

  % --- Combined signal ---
  \draw[navy, thick, domain=0:12, samples=200]
    plot (\x, {
      0.3*sin(15*\x)+0.15*cos(25*\x)+0.8
      + 1.8*exp(-(\x-2.5)^2/0.3)
      + 1.2*exp(-(\x-4.5)^2/0.15)
      + 2.2*exp(-(\x-7)^2/0.5)
      + 0.9*exp(-(\x-9.5)^2/0.2)
      + 1.4*exp(-(\x-10.8)^2/0.25)
      + 0.03*sin(50*\x) + 0.02*cos(73*\x)
    });
  \node[navy, right] at (12, 1.8) {\small Observed $\mathbf{y}$};

  % Noise annotation
  \draw[->, warningred, thick] (0.5, 1.3) -- (0.5, 0.95);
  \node[warningred, above] at (0.5, 1.3) {\small Noise $\mathbf{w}$};

  % Axes
  \draw[->, thick] (0, 0) -- (12.5, 0) node[right] {\small Time};
  \draw[->, thick] (0, 0) -- (0, 3.8) node[above] {\small Intensity};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.65, transform shape]
  ...
  \begin{scope}[xshift=0cm]
    \fill[blue!5] (-0.3,-0.5) rectangle (4.3,3.5);
    \draw[navy, thick, domain=0:4, samples=150]
      plot (\x, {1.5*exp(-(\x-2)^2/0.3) + 0.15*sin(80*\x) + 0.1*cos(120*\x)});
    \draw[warningred, thick, dashed, domain=0:4, samples=50]
      plot (\x, {-0.1 + 0.05*\x});
    \node[below, font=\small\bfseries, text=steelblue] at (2, -0.8) {Under-regularized};
    \node[below, font=\scriptsize, text=gray] at (2, -1.3) {Noisy, baseline too low};
  \end{scope}

  % Well-tuned
  \begin{scope}[xshift=5.5cm]
    \fill[forestgreen!5] (-0.3,-0.5) rectangle (4.3,3.5);
    \draw[forestgreen!80!black, thick, domain=0:4, samples=100]
      plot (\x, {1.8*exp(-(\x-2)^2/0.3)});
    \draw[forestgreen, thick, dashed, domain=0:4, samples=50]
      plot (\x, {0.1 + 0.02*\x});
    \node[below, font=\small\bfseries, text=forestgreen] at (2, -0.8) {Well-tuned};
    \node[below, font=\scriptsize, text=gray] at (2, -1.3) {Clean separation};
  \end{scope}

  % Over-regularized
  \begin{scope}[xshift=11cm]
    \fill[warningred!5] (-0.3,-0.5) rectangle (4.3,3.5);
    \draw[warningred!80!black, thick, domain=0:4, samples=100]
      plot (\x, {0.6*exp(-(\x-2)^2/0.5)});
    \draw[warningred, thick, dashed, domain=0:4, samples=50]
      plot (\x, {0.5 + 0.8*exp(-(\x-2)^2/0.6)});
    \node[below, font=\small\bfseries, text=warningred] at (2, -0.8) {Over-regularized};
    \node[below, font=\scriptsize, text=gray] at (2, -1.3) {Peaks absorbed into baseline};
  \end{scope}
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.65, transform shape,
  ...
  % CG variant
  \node[font=\small\bfseries, text=warningred] at (-3, 2) {CG Variant};
  \foreach \i in {0,...,3} {
    \node[block, draw=warningred, fill=warningred!8] (cg\i) at (\i*2.2, 2) {$K_{\text{in}}\!=\!12$};
  }
  \node at (9.5, 2) {\small$\cdots$};
  \node[block, draw=warningred, fill=warningred!8] (cg7) at (11, 2) {$K_{\text{in}}\!=\!12$};
  \node[right, font=\scriptsize, text=warningred] at (12, 2) {Depth: $8\!\times\!12=96$};

  % ISTA variant
  \node[font=\small\bfseries, text=forestgreen] at (-3, 0) {ISTA Variant};
  \foreach \i in {0,...,7} {
    \node[block, draw=forestgreen, fill=forestgreen!8] (is\i) at (\i*1.5, 0) {};
  }
  \node at (12.5, 0) {\small$\cdots$};
  \node[right, font=\scriptsize, text=forestgreen] at (13.5, 0) {Depth: $20$};
\end{tikzpicture}
\end{column}
...
\begin{tikzpicture}[scale=0.75, transform shape]
  ...
  \foreach \x/\n in {0/--,3.2/1,6.4/4,9.6/6,12.8/8,16/12} {
    \node[below=1.8cm, font=\tiny, text=gray] at (\x, 0) {\n\ terms};
  }

  \node[font=\scriptsize\itshape, text=navy] at (8, -2.8) {Progressive Development: Each experiment adds one key idea};
\end{tikzpicture}
\end{frame}

% =============================================
% SLIDE 14 — Exp 1.0: Classical BEADS
% =============================================
...
\node[right, font=\scriptsize, text=warningred] at (axis cs:5.1, 0.585) {Leakage Floor};
\end{axis}
\end{tikzpicture}
...
\begin{itemize}
  ...
  \item Synthetic-only evaluation --- no real chromatographic ground truth
  ...
  \item ${\sim}50\%$ area error --- not deployment-ready for quantitative analysis
  \item 40 test signals --- limited statistical power
\end{itemize}

\vspace{0.8em}
\begin{tcolorbox}[colback=gray!5, colframe=gray, boxrule=0.5pt]
\centering
This is a \textbf{methodological exploration}, not a production tool.
\end{tcolorbox}
\end{frame}
...
\begin{enumerate}
  ...
  \item \textbf{Group sparsity ($\ell_{2,1}$ penalty)}\\
    ...
    $\ell_1$: shrinks each element \quad vs.\quad Group: preserves within-group amplitudes.\\
    \textcolor{forestgreen}{Directly addresses the leakage mechanism.}}

  \item \textbf{Banded matrix implementation}\\
    {\small $O(N^2) \rightarrow O(N \cdot b)$.\quad Enables $N = 100{,}000+$ signals.}

  \item \textbf{Real chromatographic validation}\\
    {\small Spike-and-recovery experiments, consensus labeling.\\
    \textcolor{warningred}{The critical missing piece.}}
\end{enumerate}
\end{frame}
